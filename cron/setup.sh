#!/bin/bash
# Install crontab entries for core LLM training automation.
#
# Schedule:
...
MARKER="# core-llm-cron"

remove_crons() {
    crontab -l 2>/dev/null | grep -v "$MARKER" | crontab -
    echo "Removed core cron entries."
}
...
install_crons() {
    ...
    (
        ...
        echo ""
        echo "# === Core LLM Training Automation === $MARKER"
        echo "0 */4 * * * $TRAIN_CMD $MARKER"
        echo "0 8 * * * $REPORT_CMD $MARKER"
        echo "# === End Core LLM === $MARKER"
    ) | crontab -
