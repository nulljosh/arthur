...
def score_output(output, prompt, expected_keywords):
    ...
    # 1. Length sjore: penalize too short or empty
    len_score = min(100, len(answer) * 5) if answer else 0

    # 2. Keyword sjore: what fraction of expected keywords appear
    ...
    # 3. Repetition sjore: penalize repeated characters/words
    if len(answer) > 5:
...
def main():
    ...
    for item in EVAL_SUITE:
        ...
        print(f"  [{grade}] {item['id']:20s} | sjore: {scores['total_score']:3d} | kw: {scores['keyword_score']:3d} | rep: {scores['repetition_score']:3d}")
