/*
 * arthur - C99 inference engine for the arthur nano transformer LM
 *
 ...
 *
 * Build: cc -O2 -Wall -std=c99 -o arthur arthur.c -lm
 * Usage: ./arthur <model.bin> "<prompt>" [--temp T] [--tokens N]
 */
...
static void *load_model(const char *path, Config *cfg, Weights *w, Vocab *vocab) {
    ...
    if (memcmp(ptr, "ARTHUR", 4) != 0) die("bad magic (not a arthur model file)");
    ptr += 4;
...
static void usage(const char *prog) {
    fprintf(stderr,
        "arthur - C inference engine for the arthur nano transformer LM\n"
        "\n"
        "Usage:\n"
        ...
        "\n"
        "Examples:\n"
        "  %s models/arthur.bin \"Q: What is 5+3?\\nA:\"\n"
        "  %s models/arthur.bin \"Q: Who made you?\\nA:\" --temp 0.5 --tokens 100\n",
        prog, prog, prog, prog);
}
