#!/usr/bin/env python3
"""Train nous on comprehensive math Q&A dataset"""
...
def test_model(model, tokenizer, prompts, max_len=40, temperature=0.3):
...
DATA_FILE = 'data/math_comprehensive.txt'
MODEL_FILE = 'models/math.pt'
LOG_FILE = 'logs/math_training.log'
...
print("="*70)
print("TRAINING nous ON COMPREHENSIVE MATH DATASET")
print("="*70)
...
for epoch in range(NUM_EPOCHS):
    ...
    if (epoch + 1) % CHECKPOINT_EVERY == 0:
        checkpoint_file = f"{CHECKPOINT_DIR}/math_epoch_{epoch+1}.pt"
        save_checkpoint(model, tokenizer, epoch+1, avg_loss, checkpoint_file)
    
    # Track best model
    if avg_loss < best_loss:
        best_loss = avg_loss
        save_checkpoint(model, tokenizer, epoch+1, avg_loss, 'models/math_best.pt')
    ...
    if (epoch + 1) % TEST_EVERY == 0:
        ...
        for prompt in test_prompts[:3]:  # Test subset during training
...
print(f"✓ Best model saved to models/math_best.pt")
print("="*70)
