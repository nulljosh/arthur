...
pytestmark = pytest.mark.skipif(
    not os.path.exists('models/math_best.pt'),
...
def test_model(model_path='models/math_best.pt', temperature=0.2, max_len=50):
    ...
    for prompt in test_questions:
        ...
        # Check if answer looks reasonable (contains digits or "nous")
        answer_part = result[len(prompt):].strip()
        if answer_part and (answer_part[0].isdigit() or 'nous' in answer_part or 'I' in answer_part):
            total += 1
            ...
            if ('5+3' in prompt and '8' in answer_part) or \
               ...
               ('name' in prompt.lower() and 'nous' in answer_part) or \
               ...
               ('who are you' in prompt.lower() and 'nous' in answer_part) or \
