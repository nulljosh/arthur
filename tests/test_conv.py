...
pytestmark = pytest.mark.skipif(
    not os.path.exists('models/conversational.pt'),
...
if os.path.exists('models/conversational.pt'):
    checkpoint = torch.load('models/conversational.pt', map_location='cpu')
    tokenizer = SimpleTokenizer(checkpoint['vocab'])

    model = Arthur(vocab_size=checkpoint['vocab_size'], embed_dim=128, num_heads=4, num_layers=4, ff_dim=256, max_len=128, dropout=0.1)
